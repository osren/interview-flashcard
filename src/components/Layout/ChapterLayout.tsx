...
export function ChapterLayout({
  ...
  return (
    <div className="min-h-screen app-bg flex flex-col">
      <div className="sticky top-14 md:top-20 z-10 bg-white/95 backdrop-blur-sm border-b border-[#e5e5e5]">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-3 sm:px-4 py-2 pb-4 min-h-0">
        <div className="flex items-center justify-center w-full max-w-4xl gap-2 sm:gap-4 min-w-0">
          <button
            onClick={onPrev}
            disabled={!canPrev}
            className={cn(
              'hidden sm:flex flex-shrink-0 w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-white border-2 border-[#e5e5e5] border-b-[3px] border-b-[#d0d0d0]',
          ...
          <button
            ...
            disabled={!canNext}
            className={cn(
              'hidden sm:flex flex-shrink-0 w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-[#58CC02] border-b-[3px] border-[#46A302]',
              'items-center justify-center transition-all',
        ...
        <div className="flex sm:hidden items-center justify-center gap-8 mt-3">
          <button
            ...
            disabled={!canPrev}
            className={cn(
              'w-10 h-10 rounded-full bg-white border-2 border-[#e5e5e5] border-b-[3px] border-b-[#d0d0d0]',
              'flex items-center justify-center transition-all',
              canPrev ? 'active:border-b active:translate-y-px' : 'opacity-30 cursor-not-allowed'
            )}
          >
            <ChevronLeft size={22} className="text-[#777777]" />
          </button>
          <button
            ...
            disabled={!canNext}
            className={cn(
              'w-10 h-10 rounded-full bg-[#58CC02] border-b-[3px] border-[#46A302]',
              'flex items-center justify-center transition-all',
              canNext ? 'active:border-b active:translate-y-px' : 'opacity-30 cursor-not-allowed'
            )}
          >
            <ChevronRight size={22} className="text-white" />
          </button>
