...
export function MpxChapter() {
  ...
  useEffect(() => {
    const chapterCards = mpxCards.filter((c) => c.chapterId === chapterId);
    const mergedCards = getMergedCards('mpx', chapterId || '', chapterCards);
    setCards(mergedCards);
    const savedIndex = getCardProgress('mpx', chapterId || '');
    setCurrentIndex(Math.min(savedIndex, mergedCards.length - 1));
  }, [chapterId]);

  useEffect(() => {
    if (cards.length > 0 && chapterId) {
      saveCardProgress('mpx', chapterId, currentIndex);
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 flex flex-col pt-20">
      ...
      <ImportExportModal
        ...
        module="mpx"
        chapterId={chapterId || ''}
        title={mpxChapters.find(c => c.id === chapterId)?.title || chapterId}
      />
    </div>
  );
}