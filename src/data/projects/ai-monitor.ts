...
export const aiMonitorCards: FlashCard[] = [
  {
    id: 'ai-monitor-001',
    module: 'projects',
    chapterId: 'ai-monitor',
    category: '项目介绍',
    question: "请介绍一下你做的 AI 监控降噪项目，解决什么问题？",
    answer: `**定位**：接口错误告警的智能上报判定系统——每条告警走一次 LLM，判断「该不该上报、定什么等级」，把噪音挡在派发之前。
...
**入参硬依赖**：只有 \`url\`；其余缺失则「信息不足默认上报」。`,
  ...
  },
  {
    ...
    chapterId: 'ai-monitor',
    category: '技术选型',
    question: "为什么每条告警都走 LLM，不做规则预过滤？",
    answer: `**决策**：每条告警都走 LLM，前期不做规则旁路预过滤。
...
**面试可补充**：这是阶段一的质量优先选择；若线上成本压力大，可在「评测证明规则等价」后再加旁路，但当前明确非目标。`,
  ...
  },
  {
    ...
    chapterId: 'ai-monitor',
    category: '可靠性',
    question: "「宁可误报不可误杀」在工程上如何落地？",
    answer: `**产品原则**：漏报（FN）优先压到接近 0，误报（FP）在 FN 达标后再降。
...
**一句话**：降噪可以错报噪音，但不能把真故障静默丢掉。`,
  ...
  },
  {
    ...
    chapterId: 'ai-monitor',
    category: '路由引擎',
    question: "路由引擎怎么设计？ownedPrefixes 和 fallback 起什么作用？",
    answer: `\`createRouter(rules, { fallbackKey, ownedPrefixes })\` 流程：
...
- 阻塞主流程的非自有路径可「提升」进 ownedPrefixes，避免被归属判别压成 non-core 导致漏报`,
  ...
  },
  {
    ...
    chapterId: 'ai-monitor',
    category: 'Prompt工程',
    question: "Prompt 是怎么组织的？调判定松紧改哪里？",
    answer: `**Domain 结构**：
\`\`\`
domains/<team>/
├── rules.js
...
**注意**：Prompt 启动时读入缓存，改完需重启；上报侧尽量填全 \`url + errorCode + count\`，否则阈值形同虚设。`,
  ...
  },
  {
    ...
    chapterId: 'ai-monitor',
    category: '判定策略',
    question: "severity（P0–P3）怎么定？为什么余票查询异常可以判 P1？",
    answer: `**火车票主流程链**：查询(余票) → 填单/校验 → 下单占座 → 支付 → 出票 → 退改。
...
**反例**：展示类、NPS、公告等弱业务 → P3 或不上报。`,
  ...
  },
  {
    ...
    chapterId: 'ai-monitor',
    category: '判定策略',
    question: "business_reject（业务正常拒绝）如何识别？上线前要注意什么？",
    answer: `**定义**：接口返回了错误形态，但属于业务预期路径（无票、已退改、重复提交、待核验等），不应打扰值班。
...
**风险缓解**：discover 强制带证据 + emit 自检 + 人工回源；评测里必须保留足够「真故障」正样本。`,
  ...
  },
  {
    ...
    chapterId: 'ai-monitor',
    category: 'AI工程化',
    question: "两个 Skill（discover / onboard）分别做什么？为什么要人工确认？",
    answer: `**接入流水线**：
\`\`\`
...
**必须人工拍板**：核心链路划定、ownedPrefixes、severity 基线、business_reject 清单、rules 特例排序。生成物是草稿，上线前需回源核验。`,
  ...
  },
  {
    ...
    chapterId: 'ai-monitor',
    category: 'AI工程化',
    question: "接口清单过期会不会漏报？drift-check 解决什么问题？",
    answer: `**不会漏报**：
...
**纪律**：数据集冻结、train 调 / holdout 验、最终数字用生产模型、正负样本平衡。`,
  ...
  },
  {
    ...
    chapterId: 'ai-monitor',
    category: 'Prompt工程',
    question: "Prompt 调优是人做还是 AI 做？自动调优有什么护栏？",
    answer: `**人机协作**：
...
4. 上线前人闸门`,
  ...
  },
  {
    ...
    chapterId: 'ai-monitor',
    category: '可靠性',
    question: "LLM 网关故障或解析失败时系统会怎样？",
    answer: `**统一兜底**：\`should_report=true, severity=P2\`。
...
**相关参数**：LLM 调用 \`temperature=0\`（判定要稳定），超时约 10s。`,
  ...
  },
  {
    ...
    chapterId: 'ai-monitor',
    category: '接口设计',
    question: "HTTP 和 Kafka 双入口如何统一？对外有哪些 API？",
    answer: `**统一点**：两个入口唯一职责都是拿到一条 alarm → \`handleAlarm(alarm)\`（评测用 \`triage\`）。
...
**价值**：业务方 / 监控平台只需投递单条错误事件；调优与生产共用同一套路由和 Prompt，保证评测口径一致。`,
  ...
  },
  {
    ...
    chapterId: 'ai-monitor',
    category: '落地进展',
    question: "试点内嵌和中心化配置有什么区别？为什么分两阶段？",
    answer: `**阶段一 · 试点内嵌（当前）**：
...
**面试建议**：主动说清「判定链路已通、派发与缓存是下一阶段」，体现演进意识而不夸大上线范围。`,
  ...
  },
  {
    ...
    chapterId: 'ai-monitor',
    category: '项目理解',
    question: "这个项目里你个人贡献可以怎么讲？（结合前端实习背景）",
    answer: `**可诚实对齐的贡献叙事**（按实际参与度选用）：
...
**避免**：把「脚手架已搭」说成「已全量生产降噪并量化提升 X%」——文档未给线上降噪百分比，派发通道仍为占位。`,
  ...
  },
  {
    ...
    chapterId: 'ai-monitor',
    category: '边界澄清',
    question: "如果面试官问：降噪和故障根因定位有什么区别？",
    answer: `| 维度 | error-triage（本项目） | incident-triage |
|------|------------------------|-----------------|
...
本项目 **明确非目标**：不替代故障根因定位。降噪解决的是「注意力分配」，不是「根因分析」。`,
  ...
  },
  {
    ...
    chapterId: 'ai-monitor',
    category: '案例',
    question: "举一个端到端判定例子（面试口述用）",
    answer: `**输入**：\`/railway/train/transferLeftTicket\` 返回 \`{ errcode: "1", errmsg: "参数错误" }\`
...
| 为降噪牺牲漏报 | FN 头号指标 + 正负样本平衡纪律 |`,
  ...
  },
  {
    ...
    chapterId: 'ai-monitor',
    category: '技术选型',
    question: "和纯规则引擎方案比，LLM 方案的 tradeoff 是什么？",
    answer: `**LLM 方案优势**：
...
export const aiMonitorChapter: Chapter = {
  id: 'ai-monitor',
  module: 'projects',
  title: "AI 监控降噪工具",
  description: "接口错误告警智能上报判定：LLM 降噪、可插拔 Domain、评测闭环",
  cardCount: aiMonitorCards.length,
  icon: "📡",
};
